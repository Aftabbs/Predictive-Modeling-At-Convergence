...
def main():
    try:
        ...
        test_2024_df = df[df['AsofDate'].dt.year == 2024]
    ...
    'rf__max_depth': randint(10, 31),
    'xgb__learning_rate': uniform(0.01, 0.2),
    'xgb__max_depth': randint(6, 11),
    'lgb__learning_rate': uniform(0.01, 0.1),
    'lgb__num_leaves': randint(30, 71)
}
        ...
        random_search = RandomizedSearchCV(estimator=ensemble_model, param_distributions=param_dist, n_iter=20, cv=3, scoring='f1', n_jobs=-1, random_state=42)
        ...
        y_prob_2024 = best_model.predict_proba(X_test_2024)[:, 1]
        y_pred_2024 = (y_prob_2024 >= best_threshold).astype(int)
        test_2024_results['Probability'] = y_prob_2024
        test_2024_results['Predicted'] = y_pred_2024


    
        # with pd.ExcelWriter('Enhanced_Likely_to_launch_Model_Results.xlsx', engine='openpyxl') as writer:
        ...
        # print("Results saved to 'Enhanced_Likely_to_launch_Model_Results.xlsx'")
